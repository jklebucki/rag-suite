...
public class EmbeddingService : IEmbeddingService
{
        ...
        ILogger<EmbeddingService> logger)
    {
        ...
        // Prefix prepended to query text before embedding. e5 models require "query: ";
        // must stay consistent with the collector's passage prefix. Empty for prefix-free models (e.g. BGE-M3).
        _queryPrefix = _configuration["Services:EmbeddingService:QueryPrefix"] ?? string.Empty;
    }
    ...
    public async Task<float[]> GenerateEmbeddingAsync(string text)
    {
        try
        {
            ...
            var inputText = string.IsNullOrEmpty(_queryPrefix) ? text : _queryPrefix + text;
            var request = new EmbeddingRequest { Inputs = inputText };
