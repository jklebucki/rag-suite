...
Write-Host "RAG Collector - Orphaned Document Cleanup Test"
Write-Host "=============================================="
...
try {
    ...
    Write-Host "[OK] Connected to Elasticsearch"
    ...
    foreach ($file in $uniqueFiles.GetEnumerator()) {
        ...
        Write-Host "  $status $($file.Key) ($($file.Value) chunks)"
    ...
    Write-Host "  Total orphaned chunks: $totalOrphanedChunks"

    if ($orphanedFiles.Count -gt 0) {
        Write-Host "`nOrphaned files (would be cleaned up):"
        foreach ($orphaned in $orphanedFiles) {
            Write-Host "  - $($orphaned.FilePath) ($($orphaned.ChunkCount) chunks)"
        }

        if ($DryRun) {
            Write-Host "`n[DRY RUN MODE] No documents will be deleted."
            ...
            if ($response -eq "y" -or $response -eq "yes") {
                ...
                foreach ($orphaned in $orphanedFiles) {
                    ...
                    Write-Host "  Deleted $deleted documents"
                }
                
                Write-Host "`nTotal documents deleted: $totalDeleted"
            } else {
                Write-Host "No documents were deleted."
            }
        }
    } else {
        Write-Host "`nNo orphaned documents found. All files in the index exist on disk."
    }
